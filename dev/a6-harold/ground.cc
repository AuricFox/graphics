                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[0], &start); //Finding stroke starting point in 3D
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[stroke2d.size()-1], &end); //Finding stroke ending point in 3D
    
    Vector3 line = end - start; //Vector forming plane bottom
    Vector3 up = Vector3(0, 1, 0);

    Vector3 normal = line.Cross(up);
    Vector3 n = normal.ToUnit();
    ...
    for (int i = 0; i < stroke2d.size(); i++) {
        ...
        //printf("2D Coord: x = %f  y = %f\n", stroke2d[i].x(), stroke2d[i].y());

        Point3 proj_point = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, stroke2d[i]);

        Ray ray(eye, (proj_point - eye).ToUnit());
        ray.IntersectPlane(start, n, &time, &stroke); //Finding the location of the 2d point projected onto the plane
        silhouette_curve.push_back(stroke); //Adding 3d projected point to list
    }
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        Point3 closestP = P.ClosestPointOnPlane(start, n);
        float h = hfunc(n, silhouette_curve, closestP);
        float d = P.DistanceToPlane(closestP, n);
        
        if (h != 0) { //Adjust the vertex height if h 
            float w = std::max(0.0, 1.0 - pow((d / 5.0), 2));
            P[1] = (1 - w) * P[1] + w * h;
        }

        //Don't change Py if h equals zero
