...
void QuakeApp::DrawUsingOpenGL() {
    ...
    for (int i = start; i < end; i++) {
        ...
        double lat = GfxMath::ToRadians(e.latitude()); //Converting degrees to radians
        double lon = GfxMath::ToRadians(e.longitude());
        Point3 location = earth_.LatLongToPlane(lat, lon); //Finding location on map
        ...
        if (e.magnitude() <= 6.5) { //Low magnitude quakes
            mag = 0.02;
            r = 1.0;
            g = 1.0;
            b = 0.0;
        }
        else if (e.magnitude() > 6.5 && e.magnitude() < 8.0) { //Mid range quakes
            mag = ((e.magnitude() - 6) / 2.0) * (0.08);
            //From yellow rgb(255,255,0) to red rgb(255,0,0)
            r = 1.0;
            g = ((0.0 - 255.0) * ((e.magnitude() - 6) / 2.0) + 255.0) / 255.0;
            b = 0.0;
        }
        else if (e.magnitude() >= 8.0) { //High magnitude quakes
            mag = 0.08;
            r = 1.0;
            g = 0.0;
            b = 0.0;
        ...
        //printf("lat: %g   lon: %g   mag: %g\n", lat, lon, mag);

        Matrix4 s_quake = Matrix4::Scale(Vector3(mag, mag, mag));
        Matrix4 t_quake = Matrix4::Translation(Vector3(location[0], location[1], location[2]));
        Matrix4 blip = t_quake * s_quake;
        quick_shapes_.DrawSphere(blip, view_matrix_, proj_matrix_, Color(r, g, b));
    }
