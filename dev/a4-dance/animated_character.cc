                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    /** TODO: You will need to define a current transformation matrix for this bone that takes into account not just the parent_transform but
        also the local rotation of the bone due to the current pose. Think of the vertices that make up the geometry of each bone as being 
        defined in "bone space", where the joint that the bone rotates around is located at the origin and the bone extends in the direction
        ...
        think about what needs to happen to each vertex of a cylinder defined in "bone space" in order to get the vertex to the correct 
        ...
        perfectly with the bone's X,Y,Z axes.  The bone's rotation axes are a property of the skeleton -- they are set for each skeleton and
        do not change for each pose.  You can access a matrix that transforms from "bone space" to "rotation axis space" from the 
        skeleton_ member variable.
        ...
        "bone space".  At this point, the bone should be properly rotated based upon the current pose, but the vertices are still defined in
    ...
    */
    ...
    //skeleton_.BoneDirectionAndLength(bone_name)
    Point3 s = Point3(0, 0, 0);                                 //Starting point of bone
    Point3 e = s + skeleton_.BoneDirectionAndLength(bone_name); //Ending point of bone

    Matrix4 tb = Matrix4::Scale(Vector3(0.02f, 0.02f, 0.02f));

    Color bc = Color(1, 0.2, 0); //Bone color
    Color joint = Color(0, 0, 0); //Joint color

    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, bc, s, e, 0.01);
        Matrix4 tb = Matrix4::Scale(Vector3(0.03f, 0.03f, 0.03f));
        quick_shapes_.DrawSphere(ctm * tb, view_matrix, proj_matrix, joint);
    }
    ...
    if (bone_name == "head") {
        //quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, bc, s, e, 0.01);
        Matrix4 sc = Matrix4::Scale(Vector3(0.1f, 0.1f, 0.1f));
        quick_shapes_.DrawSphere(ctm * sc, view_matrix, proj_matrix, bc);
    }
